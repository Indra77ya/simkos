...
class commonlib {
	...
	function buildcombo_combine($arr, $key, $value1, $value2,$empty='')
	{
		$combo = array();
		if (!empty($empty)){
			$combo[''] = $empty;
	...
	function pdfHeadertemplate() {
		...
		$sHtml.="	.mydata { page-break-inside: auto; ";		
	...
	//$sHtml.="float:left;";
	$sHtml.="}";
	$sHtml.=".mydata {page-break-inside: auto; border: 1px solid #dcdcdc; margin: 10px 0px 0px 0px;  border-collapse: collapse;font-size: 9px;}";
...
function pdfFooterKuitansi() {
	...
	function reportHeader() {
		...
		$sHtml.= "<STYLE>";
		$sHtml.= 'body{padding-bottom:0;font-family:"Courier New",Helvetica,Arial,sans-serif;font-size:9px;line-height:1.5}';
		$sHtml.=".mytable { page-break-inside: auto;margin: 10px 0px 20px 0px; border: 1px solid #1e5e88; border-collapse: collapse;font-size: 9px; width:100%}";
