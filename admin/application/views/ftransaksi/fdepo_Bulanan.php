...
<tbody>
<tr ><td colspan=8 ><?php echo '<a class="btn btn-primary" href="javascript:void(0)" onclick="formDepo(this)" data-noreg="'.$res->idpendaftaran.'" data-jenis="Bulanan" data-idkamar="'.$res->idkamar.'"  data-idlokasi="'.$res->idlokasi.'" ><i class=" fa fa-credit-card" title="Bayar?"></i>&nbsp;Form Entri Mutasi</a>';?></td></tr>
...
<?	
	...
	if ($cek<=0){
		...
		foreach($result as $rs){
			...
			echo '<td><a href="javascript:void(0)" onclick="editThis(\'Bulanan\', \''.$rs->deposit_id.'\', \''.$rs->id.'\')" ><i class="glyphicon glyphicon-edit" title="Edit"></i></a>  | <a href="'.base_url('rpt_pembayaran/printDepoReceipt/Bulanan_1_'.$rs->deposit_id.'_'.$rs->id).'"><i class=" glyphicon glyphicon-print" title="Cetak Kuitansi ?"></i></a></td>';									
			echo "</tr>";
			$i++;
		}
					...
					bootbox.dialog({
					  ...
					  buttons: {
						success: {
						  ...
						  callback: function() {							
								//printReceipt();
								window.location.href="<?php echo base_url('rpt_pembayaran/printDepoReceipt/Bulanan');?>"+"_1_"+msg.id_depomaster+"_"+msg.detil_insert_id ;
						  }
						},						
						main: {
						  label: "Tutup",
