<div class="alert alert-info">
	...
	<ul>
	<li>Sewa mingguan dapat memproses pembayaran dimuka (DP), dan selanjutnya dapat melakukan pelunasan pada halaman yang sama yaitu pada menu entri pembayaran sewa mingguan</li>		
	</ul>
</div>
<?php echo form_open(null,array("class"=>"form-horizontal","id"=>"myformByr_mingguan"));?>
...
<tr>
<td >Tgl Check-In</td>
<td><input type="hidden" name="tglCekIn" value="<?php echo $res->tglmulai?>" /><B><?php echo $res->tglmulai?></B></td>	
<td align=right>Tgl Check-Out</td>
<td><input type="hidden" name="tglCekOut" value="<?php echo $res->tglakhir?>" /><b><?php echo $res->tglakhir?></b></td></tr> 
<tr bgcolor="#a7a7a7"><th colspan=4>Detil Tagihan : </th></tr>
<tr>
<td >Tarif Sewa Kamar</td>
<td  colspan=3><?php echo $res->lama." Minggu x ( Rp. ".number_format($res->tarifmingguan,2,',','.')." - Diskon Rp. ".number_format($res->diskon,2,',','.')." )  = Rp. ".number_format(($res->lama* ($res->tarifmingguan- $res->diskon)),2,',','.'); ?></td>
</tr>	
<?	
		
		$jumlah=$res->lama* ($res->tarifmingguan- $res->diskon);
...
<?php echo "<b>Rp. ".number_format($jumlah,2,',','.')."</b>"?>
...
<tr><td>Jumlah/Sisa Tagihan</td><td>:</td><td colspan=4><input type="hidden" name="tagihan" id="tagihan" value="<?=$jumlahTagihan?>"><?=number_format($jumlahTagihan,2,',','.')?></td></tr>
	...
	<INPUT TYPE="hidden" name="tarif" id="tarif" value="<?=$res->tarifmingguan?>">
...
$('#btsimpan').click(function(){		
	var form_data=$('#myformByr_mingguan').serialize();
	$.ajax({
			type: 'POST',
			url: '<?php echo base_url('e_pembayaran/saveBayarMingguan');?>',
			data: form_data,				
			dataType: 'json',
			success: function(msg) {
				...
				if(msg.status =='success'){
					...
					bootbox.dialog({
					  onEscape	:true,					 
					  message: "Data pembayaran sewa mingguan berhasil disimpan. Cetak Kuitansi ?",
					  title: "Konfirmasi",
					  buttons: {
						success: {
						  ...
						  callback: function() {
							  if ( $("#sdhdibayar").val() =="1") {								 
								  window.location.href="<?php echo base_url('rpt_pembayaran/receipt_list/Mingguan_');?>"+ $("#noreg").val()+'_'+msg.lokasi;
							  }else{
									window.location.href="<?php echo base_url('rpt_pembayaran/printReceipt/Mingguan_1_');?>"+ $("#noreg").val()+"_"+msg.detil_insert_id ;
							  }
						  }
						},						
						main: {
