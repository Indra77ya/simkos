<div class="alert alert-info">
	...
	<ul>
	<li>Sewa Bulanan saat ini sudah bisa memproses pembayaran dimuka (DP)</li>		
	<li>Pembayaran untuk 1 tagihan bisa beberapa kali, akumulasi jumlah bayar dan kekurangan untuk 1 masa tagihan tampil sesuai data tersimpan</li>		
...
$cek = $this->db->query("SELECT COUNT(*) cek, jmlbayar FROM bayar_bulanan WHERE idpendaftaran='$noreg'  and thnbln_tagihan='".$thnbln."'")->row();
...
$jmlBayar=0;$sisa=0;$kembalian=0;$status="Belum Lunas"; $kwh_akhir=0;
	if ($cek->cek >=1 && $cek->jmlbayar>0){	//proses edit, atau pembayaran belum lunas
		$rs=$this->db->query("SELECT *  FROM bayar_bulanan WHERE idpendaftaran='$noreg'  and thnbln_tagihan='".$thnbln."'")->row();
