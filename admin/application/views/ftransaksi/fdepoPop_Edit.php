<div class="alert alert-info">
	...
	<ul>
	<li>Form ini hanya untuk mengedit data pembayaran mutasi deposit berupa nominal, tanggal bayar dan metode pembayaran saja sesuai nomer pembayaran yang dipilih.</li>		
	</ul>
</div>
...
<tr ><td>Tipe mutasi </td><td>:</td><td><?=form_dropdown('tipe',array("SETORAN"=>"Setoran Masuk","PENARIKAN"=>"Penarikan/Pengembalian Dana", "value"=> $res->tipe),'','id="tipe" class="form-control"');?></td></tr>
<tr><td>Jumlah Dibayarkan</td><td>:</td><td colspan=4><input type="text" name="jmlBayar" id="jmlBayar" size="30" value="<?php echo $res->jumlah?>" onkeypress="return numericVal(this,event)" ></td></tr>
<tr ><td>Catatan </td><td>:</td><td><textarea cols=30 rows=4 name="deskripsi" id="deskripsi" ><?php echo $res->deskripsi?></textarea></td></tr>
<tr ><td colspan=2>&nbsp;</td>
	<td colspan=4>

	<INPUT TYPE="hidden" name="jenis" id="jenis" value="<?=$jenis?>">
	<INPUT TYPE="hidden" name="depo_id" id="depo_id" value="<?=$depo_id?>">
	<INPUT TYPE="hidden" name="id_detil" id="id_detil" value="<?=$id_detil?>">	
...
$('#btsimpan').click(function(){		
	...
	$.ajax({
			type: 'POST',
			url: '<?php echo base_url('e_pembayaran/saveDepoEdit');?>',
			data: form_data,				
			dataType: 'json',
			success: function(msg) {
				...
				if(msg.status =='success'){
					...
					bootbox.dialog({
					  onEscape	:true,					 
					  message: "Data mutasi deposit berhasil diupdate. Cetak Kuitansi ?",
					  title: "Konfirmasi",
					  buttons: {
						success: {
						  ...
						  callback: function() {	
							  if ( $("#jenis").val()=="Bulanan") {
								  window.location.href="<?php echo base_url('rpt_pembayaran/printDepoReceipt/Bulanan_1_');?>"+ $("#depo_id").val()+"_"+ $("#id_detil").val();

							  }else {							 
								window.location.href="<?php echo base_url('rpt_pembayaran/printDepoReceipt/Tahunan_1_');?>"+ $("#depo_id").val()+"_"+ $("#id_detil").val();
							  }
						  }
						},						
						main: {
