...
<?php echo form_open(null,array("class"=>"form-horizontal","id"=>"myform_insidentil"));?>
<div class="widget-box">
	<div class="widget-header">
		<h4 class="widget-title smaller">Form Entri Pengeluaran Insidentil</h4>
	</div><!-- widget-header -->
...
<div class="row">
	<div class="col-md-8">
		<div class="form-group"><label  class="col-sm-4 control-label">Untuk Pembayaran</label>
			<div class="col-sm-8"><?=form_textarea(array("name"=>"keterangan","id"=>"keterangan","class"=>"form-control", "value"=>($res==""?"":$res->keterangan), "rows"=>3, "cols"=>30));?>	</div>
		</div>
	</div>
</div>
...
$('#btsimpan').click(function(){		
		var form_data = $('#myform_insidentil').serialize();
		$().showMessage('Sedang diproses.. Harap tunggu..');
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('e_pengeluaran/savePengeluaran_ins');?>',
			data: form_data,				
			dataType: 'json',
			success: function(msg) {
				...
				if(msg.status =='success'){
					$().showMessage('Data Pengeluaran Insidentil berhasil disimpan.','success',1500);
					if ($("#state").val()=="add"){
						window.location.href="<?php echo base_url('e_pengeluaran/insidentil');?>" ;
					}else{
