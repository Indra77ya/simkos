...
class rpt_pembayaran extends MY_App {
	...
	public function unpaidToExcel(){
		...
		$reslokasi =$this->transaksi_model->getLokasi($lokasi);
		$namafile="tagihanList_".$reslokasi->lokasi."_".date('Y-m').".csv";
		$filepath = 'public/report/'.$namafile;

		// Open file for writing
		$fp = fopen($filepath, 'w');
		if ($fp === false) {
			$data = array('status' => 'error', 'message' => 'Unable to open file for writing: ' . $filepath);
		...
		// Add Header Row
		fputcsv($fp, array('NO', 'Nama Kamar', 'Nama Penghuni', 'Tgl Mulai Inap', 'Detil Tagihan'));
		...
		if ($jenis_sewa=="Bulanan"){
			...
			foreach($query as $rowXls){
				...
				// Calculate logic simplified for CSV
