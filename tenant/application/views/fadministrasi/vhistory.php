...
<?	//echo $strData;
	if ($display=="0"){
		echo '<a href="'.base_url("administrasi/history/".$jns_sewa."_1_".$noreg."_".$lokasi->id).'" title="Generate to pdf"><img src="'.base_url("assets/images/oficina_pdf.png").'" width=40 ><B><font size="2" style="Courier New" >Cetak ke Pdf</font></B></a>&nbsp;<BR>';
	}
	
	echo '<div class="row" ><div class="col-md-6">';
	echo "<table class='".($display=="0"?"table table-bordered table-hover":"mytable")."'>"; 
	//echo "<tr><th colspan=\"4\">$strData</th></tr>";
	echo "<tr> <td width='25%'>No. Registrasi</td><td colspan=\"3\"><b>".$rsData->idpendaftaran."</b></td> </tr>";
	echo "<tr> <td>Nama</td> <td colspan=\"3\"><b>".$rsData->nama."</b></td> </tr>";
	echo "<tr valign=top> <td>Alamat/Telp</td>  <td colspan=\"3\"><b>".$rsData->alamat_asal."<br>".($jns_sewa=="Bulanan" || $jns_sewa=="Tahunan"?$rsData->notelp:$rsData->telp)."</b></td> </tr>";
	echo "<tr> <td>Sewa Kamar</td> <td colspan=\"3\"><b>".$rsData->labelkamar."</b></td> </tr> "; 
	if ($jns_sewa=="Harian"|| $jns_sewa=="Mingguan"){
		echo "<tr> <td>Total Tagihan</td>  <td colspan=\"3\"><b>Rp. ".number_format($tagihan,2,',','.')."</b></td> </tr> "; 
	}
	$tglmulai=($jns_sewa=="Harian"?$rsData->tglcek_in:($jns_sewa=="Tahunan"?$rsData->tgldaftar:$rsData->tglmulai));
	echo "<tr> <td>Tanggal Cek In</td> <td colspan=\"3\"><b>".strftime("%d-%m-%Y", strtotime($tglmulai))."</b></td> </tr>";
	echo "<tr><td>Status</td>  <td colspan=\"3\"><b>".($rsData->checkout=="0"?"Check In":"Check Out").($jns_sewa=="Harian"|| $jns_sewa=="Mingguan"?", ".$status:"")."</b></td> </tr>";
	echo "</table></div></div>";

	echo "<table class='".($display=="0"?"table table-bordered table-hover":"mytable")."' >"; 
	echo "<tr><th ".($jns_sewa=="Bulanan" || $jns_sewa=="Tahunan"?"colspan=7":" colspan=4").">DATA PEMBAYARAN ".($jns_sewa=="Tahunan"?" UANG SEWA":"")."</th> </tr>";
	echo "<tr><th>No".($jns_sewa=="Bulanan" || $jns_sewa=="Tahunan"?". Kuitansi </th><th>Periode Tagihan</th>":"")."</th>".($jns_sewa=="Bulanan" || $jns_sewa=="Tahunan"?"<th>Kamar</th><th>Tagihan & Denda</th>":"")."<th>Jml Bayar</th><th>Tgl Bayar</th><th >Data Pembayaran</th></tr>";
	//data pembayaran 
	...
	if ($jns_sewa=="Harian"|| $jns_sewa=="Mingguan"){
	...
	}elseif ($jns_sewa=="Bulanan"){	//bulanan
		if ($this->db->query($qryRows)->num_rows <=0){
		...
		foreach( $resBayar as $rsMaster){	
					...
					foreach( $resDetil as $rsRows){	
						...
						echo "<td align=center>".$arrBulan[$idxBln]." ".$idxThn."</td>";
	...
	}else{	//tahunan log sewa dan tag bulanan
		if ($this->db->query($qryRows)->num_rows <=0){
			echo "<tr><td colspan=7 align=center>Belum Ada pembayaran</td></tr>";
		}else{
		...
		foreach( $resBayar as $rsMaster){
					...
					foreach( $resDetil as $rsRows){	
						echo "<tr><td align=center>".$rsRows->no_urut." </td>";
						echo "<td align=center>".$rsMaster->tahun_tagihan."</td>";
						echo "<td >".$rsMaster->namaKamar."</td>"; //nama kamar
						//$tagihan=($rsRows->jmlTagihan + $rsRows->denda ) + $rsRows->pengurangan_denda;
						$tagihan=$rsMaster->jmlTagihan ;
						//detil bayar
			...
			echo "<tr><th  colspan=3>Total </th><th>Rp. ".number_format($tagListrik,2,',','.')."</th><th>Rp. ".number_format($tagPDAM,2,',','.')."</th><th>Rp. ".number_format($jmlBayar,2,',','.')."</th><th colspan=3></th></tr>";
		}
	}
...
<div class="row" style="text-align:center">
	<div class="col-md-12">	<input type="hidden" name="param" id="param" value="<?=$param?>">
		...
		<a href="<?=base_url("administrasi/history/".$jns_sewa."_1_".$noreg."_".$lokasi->id)?>" class="btn btn-success">Cetak/Download</a><br>		
	</div>
</div>	
<?}?>
<script>
